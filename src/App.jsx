...
function DiagramApp() {
  ...
  useEffect(() => {
    ...
    setBlockTypes((prev) => {
      ...
      const next = [...withoutAuto, ...autoComponents];
  ...
  const openEdit = (t) => {
    if (t?.autogeneratedFromFrameId) return;
    setEditingId(t.id);
  ...
  const updateNodeAttr = (name, value) => {
    setNodes((prev) =>
      prev.map((n) =>
        n.id === selectedNodeId
          ? (autogeneratedTypeIds.has(n.data?.typeId)
            ? n
            : {
                ...n,
                data: {
  ...
  const openInstanceEditorForNode = useCallback((node) => {
    ...
    if (autogeneratedTypeIds.has(node.data?.typeId)) return;
  ...
  const saveInstanceEditor = () => {
    ...
    if (selectedNode?.type === 'block' && autogeneratedTypeIds.has(selectedNode.data?.typeId)) return;
    setNodes((prev) =>
      prev.map((n) =>
        n.id === selectedNodeId
          ? {
                ...
                {grouped[hoveredGroup].map((t) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => openEdit(t)}
                      title={t.autogeneratedFromFrameId ? 'Тип компонента генерируется автоматически' : 'Редактировать тип'}
                      aria-label="Редактировать тип"
                      disabled={Boolean(t.autogeneratedFromFrameId)}
                    >
              ...
              <>
                ...
                <div className="inspector-actions">
                  <button
                    ...
                    title={isSelectedComponentNode ? 'Экземпляр компонента генерируется автоматически' : 'Редактировать экземпляр'}
                  ...
                  <button
                    ...
                    title={selectedType?.autogeneratedFromFrameId ? 'Тип компонента генерируется автоматически' : 'Редактировать тип'}
                    aria-label="Редактировать тип"
                    disabled={!selectedType || Boolean(selectedType?.autogeneratedFromFrameId)}
                  >
                    <span className="material-symbols-outlined">edit</span>
                  </button>
                </div>
                  ...
                  <div className="inspector-desc__section">
                    ...
                    {selectedNode.data.attributes.map((a) => (
                      <label className="field field--compact" key={a.name}>
                        ...
                        <input value={a.value} onChange={(e) => updateNodeAttr(a.name, e.target.value)} disabled={isSelectedComponentNode} />
                      </label>
                    ))}
                  </div>
                </div>
