...
const isDefaultAttributeName = (name) => {
  const clean = String(name || '').trim().toLowerCase();
  return clean === 'id' || clean === 'name';
};

const ensureDefaultTypeAttributes = (attrs = []) => {
  const custom = attrs
    .filter((a) => !isDefaultAttributeName(a.name))
    .map((a) => ({ name: String(a.name || '').trim(), hidden: Boolean(a.hidden) }))
    .filter((a) => a.name);

  return [...DEFAULT_ATTRIBUTES.map((a) => ({ ...a })), ...custom];
};

const ensureDefaultNodeAttributes = (attrs = []) => {
  const existingByName = new Map(attrs.map((a) => [String(a.name || '').trim().toLowerCase(), a]));
  const defaultsWithValues = DEFAULT_ATTRIBUTES.map((base) => {
    const existing = existingByName.get(base.name.toLowerCase());
    return {
      name: base.name,
      hidden: true,
  ...
  return [...defaultsWithValues, ...custom];
};

const arePortsEqual = (a = [], b = []) => {
...
const areTypeAttributesEqual = (a = [], b = []) => {
  if (a.length !== b.length) return false;
  for (let i = 0; i < a.length; i += 1) {
    if (a[i].name !== b[i].name || Boolean(a[i].hidden) !== Boolean(b[i].hidden)) return false;
  }
  return true;
};

const areNodeAttributesEqual = (a = [], b = []) => {
  if (a.length !== b.length) return false;
  for (let i = 0; i < a.length; i += 1) {
    if (a[i].name !== b[i].name || Boolean(a[i].hidden) !== Boolean(b[i].hidden) || (a[i].value || '') !== (b[i].value || '')) return false;
  }
  return true;
};
...
const ensureCoreBlockTypes = (types = []) => {
  const normalized = types.map((t) => ({
    ...t,
    attributes: ensureDefaultTypeAttributes(t.attributes || []),
  }));
...
function DiagramApp() {
  ...
  useEffect(() => {
    ...
    for (const frameNode of frameNodes) {
      ...
      autoComponents.push({
        ...
        inputs,
        outputs,
        attributes: ensureDefaultTypeAttributes([]),
    ...
    setBlockTypes((prev) => {
      ...
      for (let i = 0; i < prev.length; i += 1) {
        ...
        if (
          ...
          a.autogeneratedFromFrameId !== b.autogeneratedFromFrameId ||
          !arePortsEqual(a.inputs || [], b.inputs || []) ||
          !arePortsEqual(a.outputs || [], b.outputs || []) ||
          !areTypeAttributesEqual(a.attributes || [], b.attributes || [])
        ) {
          return next;
        }
      }
  ...
  useEffect(() => {
    ...
    setNodes((prev) => {
      ...
      const next = prev.map((n) => {
        ...
        if (!typeDef?.autogeneratedFromFrameId) {
          const normalizedAttrs = ensureDefaultNodeAttributes(n.data.attributes || []);
          if (areNodeAttributesEqual(normalizedAttrs, n.data.attributes || [])) return n;
          changed = true;
          return { ...n, data: { ...n.data, attributes: normalizedAttrs } };
        }

        const nextInputs = deepClone(typeDef.inputs || []);
        const nextOutputs = deepClone(typeDef.outputs || []);
        const nextAttributes = ensureDefaultNodeAttributes(
        ...
        const nextIcon = String(n.data.icon || '').trim() || typeDef.icon || '';
        const hasChanges =
          n.data.blockGroup !== typeDef.group ||
          n.data.blockName !== typeDef.name ||
          n.data.instanceName !== typeDef.name ||
          (n.data.icon || '') !== nextIcon ||
          (n.data.headerColor || '') !== (typeDef.headerColor || '#E2E8F0') ||
          !arePortsEqual(n.data.inputs || [], nextInputs) ||
          !arePortsEqual(n.data.outputs || [], nextOutputs) ||
          !areNodeAttributesEqual(n.data.attributes || [], nextAttributes);
  ...
  const selectSingleNode = useCallback((nodeId) => {
    ...
    setNodes((prev) => {
      ...
      const next = prev.map((n) => {
        ...
        if (Boolean(n.selected) === nextSelected) return n;
        changed = true;
  ...
  const instanceEditingNode = selectedNodeId ? nodeById.get(selectedNodeId) : null;
  ...
  const selectedDefaultAttrs = (selectedNode?.data?.attributes || []).filter((a) => isDefaultAttributeName(a.name));
  const selectedCustomAttrs = (selectedNode?.data?.attributes || []).filter((a) => !isDefaultAttributeName(a.name));
  ...
  const instantiate = (typeDef) => {
    const node = {
      ...
      data: {
        ...
        attributes: ensureDefaultNodeAttributes(
          typeDef.attributes.map((attr) => ({
            name: attr.name,
  ...
  const openEdit = (t) => {
    ...
    setDraftAttrs(
      t.attributes
        .filter((attr) => !isDefaultAttributeName(attr.name))
        .map((attr) => ({ name: attr.name, hidden: Boolean(attr.hidden) }))
    );
    setEditorOpen(true);
    closeMenu();
  };
  ...
  const openInstanceEditorForNode = useCallback((node) => {
    ...
    setInstanceDraftAttrs(
      deepClone((node.data.attributes || []).filter((a) => !isDefaultAttributeName(a.name)))
    );
        ...
        n.id === selectedNodeId
          ? {
              ...
              data: {
                ...
                attributes: ensureDefaultNodeAttributes([
                  ...(n.data.attributes || []).filter((a) => isDefaultAttributeName(a.name)),
                  ...instanceDraftAttrs.filter((a) => a.name?.trim()).map((a) => ({ ...a, name: a.name.trim() })),
                ]),
              },
            }
          : n
            ...
            if (event.shiftKey) {
              ...
              setNodes((prev) => {
                ...
                const next = prev.map((node) => {
                  if (node.id !== n.id || node.selected) return node;
                  changed = true;
            ...
            {selectedNode.type === 'block' ? (
              <>
                ...
                <div className="inspector-meta-card">
                  {selectedDefaultAttrs.map((a) => (
                    <label className="field field--compact" key={`default-${a.name}`}>
                      <span>{a.name}</span>
                      <input value={a.value} onChange={(e) => updateNodeAttr(a.name, e.target.value)} />
                    </label>
                  ))}
